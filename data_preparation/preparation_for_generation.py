...
def generate_training_string(articles):
    ...
    for article in articles:
        if article['explanation'] == '' or article['explanation'].isspace() or len(article['explanation']) < 6:
            continue
        if article['annotations'] == None or len(article['annotations']) == 0:
            continue
        train_str += '<|startoftext|>' + '\n'
        # train_str += article['article'] + '\n'
        # train_str += article['explanation_textrank'] + '\n'
        annotated_sentences = []
        for index in article['annotations']:
            annotated_sentences.append(article['sentences'][index])
        train_str += ' '.join(annotated_sentences) + '\n'
...
def prepare_for_generation_per_question_satisfactory():
    for i in range(1, 2):
        ...
        with open('../data/annotated_data_processed/q{}_train.json'.format(i)) as train_file:
            train_set = json.load(train_file)
        ...
        with open('../data/generation_input/annotated_textrank/q{}_sat.txt'.format(i), 'w') as f:
            f.write(train_str)
